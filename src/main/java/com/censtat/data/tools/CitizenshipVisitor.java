...
public class CitizenshipVisitor implements ModuleVisitorInterface {
	...
	public FileWriter visit(DataEntity entity, FileWriter writer) {
		...
		createCitizenshipPage(entity);
		return writer;
	}
	
	private void createCitizenshipPage(DataEntity entity) {
		System.out.println("***********Creating the Citzenship Page ******************");
		//String toCreate = CenPageCreationContext.getInstance().getCurrentWorkingDirectory() + File.separator
		//		+ CenModulePageCreator.getInstance().getLinkName(entity.getState());
		...
		if (entity.getDataEntityType().equals(DataEntityTypeInterface.CITY_TYPE)) {
			...
			CenModulePageCreator.getInstance().addModuleInfo(CenModulePageCreator.MODULE_USCITIZENSHIP, toCreate, entity);
			return;
		}
