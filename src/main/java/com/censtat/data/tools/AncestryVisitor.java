...
public class AncestryVisitor implements ModuleVisitorInterface {
	...
	/*	template.setKeyMap(entity.getItentifierKeys());
		...
		createAncestryPage(entity);
		return writer;
	}
	
	private void createAncestryPage(DataEntity entity) {
		System.out.println("***********Creating the Ancestry Page ******************");
		...
		if (entity.getDataEntityType().equals(DataEntityTypeInterface.CITY_TYPE)) {
			...
			CenModulePageCreator.getInstance().addModuleInfo(CenModulePageCreator.MODULE_ANCESTRY, toCreate, entity);
			return;
		}
