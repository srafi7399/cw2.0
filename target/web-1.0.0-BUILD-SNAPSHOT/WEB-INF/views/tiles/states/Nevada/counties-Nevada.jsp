
<div class="content-wrapper">
        <!-- Content Header (Page header) -->  
   


        <!-- Main content -->
        <section class="content">   
         <section class="content-header">    
	<div class="row">      
          <ol class="breadcrumb">
		            <li><a href="/"><i class="fa fa-dashboard"></i> Home</a></li>
		            <li><a href="/states"><i class="fa fa-dashboard"></i> states</a></li>
		            <li><a href="/states/Nevada"><i class="fa fa-dashboard"></i> Nevada</a></li>
            <li class="active">counties-Nevada</li>
          </ol>
 </div>
	</section>
 
 


<div class="row">
<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
<!-- below-sex-data-horizontal -->
<ins class="adsbygoogle"
     style="display:block"
     data-ad-client="ca-pub-3154845825068093"
     data-ad-slot="3578399368"
     data-ad-format="auto"></ins>
<script>
(adsbygoogle = window.adsbygoogle || []).push({});
</script>
</div>

<div class="table-responsive">
  <table class="table table-condensed table-striped table-bordered table-hover no-margin">
    <thead>
      <tr>        
        <th style="width:10%">Name</th>
        <th style="width:10%" >Population</th> 
         <th style="width:10%" >Male Vs Female (%)</th>  
         <th style="width:10%" >Median Age</th>  
         <th style="width:10%" >Population with Bachelors or higher degrees (%)</th>    
         <th style="width:10%" >Hispanic Population(%)</th>        
        <th style="width:10%"">Average Family Size</th>
      </tr>
    </thead>
    <tbody>    
      <tr>       
        <td>        
          <span class="name"> <a href="/states/Nevada/counties/Carson-City">Carson City</a></span>
        </td>        
        <td>54481     
        </td>
        <td>51.2% Male and 48.8% female
        </td>
        <td>42 years        
        </td>
        <td>19.8
        </td>
        <td>22.4 
        </td>
        <td> 3.19
        </td>        
      <tr>       
        <td>        
          <span class="name"> <a href="/states/Nevada/counties/Nye-County">Nye County</a></span>
        </td>        
        <td>42842     
        </td>
        <td>50.1% Male and 49.9% female
        </td>
        <td>50.4 years        
        </td>
        <td>14.2
        </td>
        <td>14 
        </td>
        <td> 2.89
        </td>        
      <tr>       
        <td>        
          <span class="name"> <a href="/states/Nevada/counties/Churchill-County">Churchill County</a></span>
        </td>        
        <td>24333     
        </td>
        <td>49.8% Male and 50.2% female
        </td>
        <td>39.5 years        
        </td>
        <td>15.2
        </td>
        <td>12.9 
        </td>
        <td> 3.07
        </td>        
      <tr>       
        <td>        
          <span class="name"> <a href="/states/Nevada/counties/Douglas-County">Douglas County</a></span>
        </td>        
        <td>47057     
        </td>
        <td>50.4% Male and 49.6% female
        </td>
        <td>48.6 years        
        </td>
        <td>24.9
        </td>
        <td>11.6 
        </td>
        <td> 2.79
        </td>        
      <tr>       
        <td>        
          <span class="name"> <a href="/states/Nevada/counties/Clark-County">Clark County</a></span>
        </td>        
        <td>1997371     
        </td>
        <td>50.2% Male and 49.8% female
        </td>
        <td>36.1 years        
        </td>
        <td>22.1
        </td>
        <td>29.7 
        </td>
        <td> 3.45
        </td>        
      <tr>       
        <td>        
          <span class="name"> <a href="/states/Nevada/counties/Elko-County">Elko County</a></span>
        </td>        
        <td>50904     
        </td>
        <td>52.1% Male and 47.9% female
        </td>
        <td>33.4 years        
        </td>
        <td>17.2
        </td>
        <td>23.6 
        </td>
        <td> 3.39
        </td>        
      <tr>       
        <td>        
          <span class="name"> <a href="/states/Nevada/counties/Washoe-County">Washoe County</a></span>
        </td>        
        <td>429241     
        </td>
        <td>50.4% Male and 49.6% female
        </td>
        <td>37.4 years        
        </td>
        <td>28.1
        </td>
        <td>23 
        </td>
        <td> 3.24
        </td>        
      <tr>       
        <td>        
          <span class="name"> <a href="/states/Nevada/counties/Lyon-County">Lyon County</a></span>
        </td>        
        <td>51450     
        </td>
        <td>50.1% Male and 49.9% female
        </td>
        <td>42.1 years        
        </td>
        <td>17.8
        </td>
        <td>15.4 
        </td>
        <td> 3.06
        </td>        
         </tbody>
  </table>
</div>     
</section>
</div>                       		