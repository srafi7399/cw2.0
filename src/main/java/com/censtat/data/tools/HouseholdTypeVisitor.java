...
public class HouseholdTypeVisitor implements ModuleVisitorInterface {
	...
	private void createHouseHoldsByTypePage(DataEntity entity) {
		System.out.println("***********Creating the HouseHolds By Type******************");
		...
		if (entity.getDataEntityType().equals(DataEntityTypeInterface.CITY_TYPE)) {
			...
			CenModulePageCreator.getInstance().addModuleInfo(CenModulePageCreator.MODULE_HOUSEHOLDS_BY_TYPE, toCreate, entity);
			return;
		}
	...
	private void buildandStoreGUICharts(DataEntity entity) {
		...
		CenPieChart pieChart0 = asBuilder.getAgeSexPieChart(entity,
				"name_fam_vs_nonfam_hos");
		
		storeTheCharts(pieChart0, entity,"name_fam_vs_nonfam_hos");
