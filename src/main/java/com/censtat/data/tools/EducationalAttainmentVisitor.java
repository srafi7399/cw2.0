...
public class EducationalAttainmentVisitor implements ModuleVisitorInterface {
	...
	private void createEducationalAttainmentPage(DataEntity entity) {
		...
		if (entity.getDataEntityType().equals(DataEntityTypeInterface.CITY_TYPE)) {
			...
			CenModulePageCreator.getInstance().addModuleInfo(CenModulePageCreator.MODULE_EDUCATIONALATTAINMENT, toCreate, entity);
			return;
		}
	...
	private void buildandStoreGUICharts(DataEntity entity) {
		...
		CenPieChart pieChart0 = asBuilder.getAgeSexPieChart(entity,
				"name_educational_attainment_levels");		
		storeTheCharts(pieChart0, entity,"name_educational_attainment_levels");
	}
